...
class Scanner():
    ...
    def _conn(self):
        try:
            if self.scheme == 'https':
                conn = urllib3.HTTPSConnectionPool(self.netloc)
            else:
                conn = urllib3.HTTPConnectionPool(self.netloc)
            return conn
    ...
    def _get_status(self, path):
        ...
        for i in range(0, 3):
            try:
                conn = self._conn()
                status = conn.request('OPTIONS', path).status
                conn.close()
                return status
            except Exception as e:
